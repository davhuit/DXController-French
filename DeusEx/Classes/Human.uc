...
function PlayWalking()
{
	...
	// Walk anim assumed authored at ~half run pace; tune GamepadWalkAnimSpeedFraction
	// (declared on DeusExPlayer) if foot-slide is visible at full-cadence walk.
	if (GamepadStickMag > 0.0)
	{
		humanAnimRate = Default.humanAnimRate * (CurrentSpeedFraction() / GamepadWalkAnimSpeedFraction);
		humanAnimRate = FClamp(humanAnimRate, 0.25, 2.0);
	}
...
function PlayRunning()
{
	...
	if (GamepadStickMag > 0.0)
	{
		humanAnimRate = Default.humanAnimRate * CurrentSpeedFraction();
